        ...
        <div class="g-details container left">
          <div class="m-header">
            <div class="m-header title">
              <strong>{{details.blog.title}}</strong>
            </div>
...
<script>
  ...
  import { Menu, MenuItem, Card, Tag, Icon, Button, Input } from 'iview';
  ...
  export default {
    ...
    methods: {
      ...
      refreshData (href) {
        ...
        this.$ajax.get('/api/blog/details?' + query).then(function (res) {
      ...
      commentSubmit () {
        ...
        this.comment = '';
        this.$ajax.post('/api/comment/submit', commentData).then(function (res) {
          if (res.data.status === 3200) {
      ...
      getArticleList (mode) {
        ...
        this.$ajax.post('/api/blog/query?limit=20', {
          request: request,
          carousel: false
        }).then(function (res) {
