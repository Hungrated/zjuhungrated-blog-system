...
<script>
  ...
  import { Menu, MenuItem, Icon, Card, Button, Select, Option, Upload, Page, Input, Tag, Table } from 'iview';
  ...
  export default {
    ...
    methods: {
      ...
      submitFile () {
        ...
        this.$ajax.post('/api/file/upload', formData, this.uploadConfig).then(function (res) {
      ...
      refreshFileList (mode) {
        ...
        this.$ajax.post('/api/file/query?limit=' + this.pageLimit + '&page=' + this.curPage, {
          request: request
        }).then(function (res) {
