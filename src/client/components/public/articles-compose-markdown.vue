...
<script>
  import { Card, Icon, Upload, Button } from 'iview';

  export default {
    name: 'markdown-editor',
    props: ['title', 'group', 'labels', 'description'],
    components: {
      Card, Icon, Upload, Button
    },
    data () {
      return {
        value: '',
    ...
    methods: {
      ...
      uploadImg (blogId, type, uploaderId) {
        ...
        let _this = this;
        this.$ajax.post('/api/image/upload', formData, this.uploadConfig).then(function (res) {
      ...
      importText (file) {
        ...
        this.$ajax.post('/api/blog/import', formData, this.uploadConfig).then(function (res) {
          _this.$Message.success(res.data.msg);
      ...
      submit () {
        ...
        if (!submitData.title ||
          ...
          const _this = this;
          this.$ajax.post('/api/blog/publish', submitData).then(function (res) {
