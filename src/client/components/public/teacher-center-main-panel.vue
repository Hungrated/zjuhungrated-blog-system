              ...
              <div class="m-import body">
                ...
                <div class="m-import body right">
                  <p>
                    ...
                    上传教务系统里下载的教学班点名册以导入班级与学生信息&emsp;
                    <Button type="success"
                            size="small"
                            @click="downloadEmptyTable()"
                            icon="archive">下载空表
                    </Button>
                  </p>
                </div>
...
<script>
  import { Card, Upload, Button, Icon, Table, Modal, Timeline, TimelineItem } from 'iview';

  export default {
    name: 'teacher-center-main-panel',
    components: {
      Card, Upload, Button, Icon, Table, Modal, Timeline, TimelineItem
    },
    data () {
      return {
        classParse: false,
    ...
    methods: {
      ...
      downloadEmptyTable () {
        this.downloadFile('/api/achieves/emptytable/empty-table.xls');
      },
      downloadFile (url) {
      ...
      parseClassData () {
        ...
        this.$ajax.post('/api/user/parse', formData, this.uploadConfig).then(function (res) {
      ...
      fetchMoments () {
        let _this = this;
        this.$ajax.get('/api/moment/fetch?type=planmod&limit=40').then(function (res) {
