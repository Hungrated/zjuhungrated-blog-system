 ...
 * }
 */
router.post('/publish', validate.validateUser, function (req, res) {
  // publish a blog(project or event)
  let publishData = req.body;
  publishData.blog_id = 'blg' + uid.generate();
  publishData.author_id = req.body.school_id;
  ...
  publishData.labels = `,${publishData.labels},`;
 ...
 * }
 */
router.post('/import', objMulter.any(), validate.validateUser, function (req, res, next) {
  // upload text file
...
router.post('/query', function (req, res) {
  ...
  Blog.findAndCount(queryTotal)
    .then(function (data) {
      let articles = data.rows;
      for (let i = 0; i < articles.length; i++) {
        articles[i].dataValues.publishTime = timeFormat(articles[i].dataValues.created_at);
        articles[i].dataValues.labels = articles[i].dataValues.labels.slice(1);
      }
      let count = 0;
      let carouselList = [];
      if (req.body.carousel) {
        for (let i = 0; i < articles.length; i++) {
          let item = articles[i].dataValues;
          if (item.cover) {
            item.index = count++;
            item.labels = item.labels.slice(1);
